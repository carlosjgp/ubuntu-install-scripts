...
mv terraenv /usr/local/bin/terraenv && \
...
# Install tfsec
curl -sLo /usr/local/bin/tfsec https://github.com/liamg/tfsec/releases/download/${TFSEC_VERSION}/tfsec-linux-amd64 && \
...
# Install goss
curl -sLo /usr/local/bin/goss https://github.com/aelsabbahy/goss/releases/download/${GOSS_VERSION}/goss-linux-amd64 && \
...
tar -C /usr/local/bin/ -xzf /tmp/run.tar.gz

# Install yq
curl -sLo /usr/local/bin/yq https://github.com/mikefarah/yq/releases/download/3.2.1/yq_linux_amd64 && \
chmod 755 /usr/local/bin/yq

# Install sops
curl -sLo /usr/local/bin/sops https://github.com/mozilla/sops/releases/download/v3.5.0/sops-v3.5.0.linux && \
...
curl -sSL -o trivy-junit.tpl https://raw.githubusercontent.com/aquasecurity/trivy/master/contrib/junit.tpl
